...
class Tester(TesterInterface):
    ...
    def get_users_without_mfa(self, organization):
        result = []
        raw_api_result_all_users = requests.get(headers=self.request_headers,
                                                url='https://api.github.com/orgs/' + organization + '/members')
        raw_api_result_all_users_obj = raw_api_result_all_users.json()
        raw_api_result_2fa_disabled = requests.get(headers=self.request_headers,
                                                   url='https://api.github.com/orgs/' + organization + '/members?filter=2fa_disabled')
    ...
    def get_forkable_repositories(self, organization):
        result = []
        raw_api_result = requests.get(headers=self.request_headers,
    ...
    def check_for_too_many_admin_users(self, organization):
        ...
        org_admins = []
        raw_api_result = requests.get(headers=self.request_headers,
                                      url='https://api.github.com/orgs/' + organization + '/members?role=admin')
        raw_api_result_obj = raw_api_result.json()
